...
%                        (default=[])
%      'CBThickness'  Colorbar thickness
%                        (default = 1)
...
%                        'off' (default) | 'on' | 'only' (will disable color levels)
...
%                        'auto' (default) | [N,M]
%
%       'Deform' ->  possible deformation field (cell array of two fields), added for StabFem (patch DF 3/12/2021)
...
function [hh,varargout] = ffpdeplot(points,boundary,triangles,varargin)
    ...
    optsnames = {'VhSeq', 'XYData', 'XYStyle', 'Mesh', 'MColor', 'Boundary', 'BDLabels', ...
                 ...
                 'ColorMap', 'ColorBar', 'CBTitle', 'CBThickness', 'ColorRange', ...
                 'Title', 'XLim', 'YLim', 'ZLim', 'DAspect', ...
                 'FlowData', 'FColor', 'FGridParam','ColorRangeTicks','Deform'};
    ...
    [vhseq, xyrawdata, xystyle, showmesh, mcolor, showboundary, bdlabels, ...
     ...
     setcolormap, showcolbar, colorbartitle, colorbarthickness, colorrange, ...
     plottitle, plotxlim, plotylim, plotzlim, axisaspect, ...
     flowrawdata, fcolor, fgridparam,logscaleS,Deform] = vararginval{:};
    if strcmp(contourplt,'only')
        contourplt = 'on';
        xystyle = 'off';
        colorrange ='off';
        setcolormap ='off';
        showcolbar ='off';
        contourstyle = 'patchdashedneg';
    end
    ...
    if isempty(Deform)
        ...
        % this patch is for usage with StabFem for cases involving mesh deformation
        if ~iscell(Deform)
            error('something went wrong when transmtting amplitude of deformation');
        end
        xix = Deform{1};
        xiy = Deform{2};
        [xmesh,xpts,ymesh,ypts] = SF_prepare_mesh(points,triangles,xix,xiy);
    end
    ...
    if ~isempty(xyrawdata)
        ...
        %Contour Plot
            ...
            if exist('fftri2gridfast','file')
                ...
                if (N>100)&&(exist('OCTAVE_VERSION', 'builtin')) % second part of the test Added for StabFem
            ...
            if (isnumeric(colorrange))
                ...
                if strcmpi(colorrange,'minmax') || strcmpi(colorrange,'centered')
                    ...
                    if strcmpi(colorrange,'minmax')
                        if min(min(cdata))-max(max(cdata))==0
                            CAXIS = [-1 1];
                            warning(' problem when detecting color range : fields seems constant');
                        else
                    ...
                    if (isempty(crng) || (min(crng) == max(crng)))
                        ...
                        warning('''ColorRange'': No color spread in the cropped section / try ''minmax''');
                    end
        ...
        if ~(strcmpi(showcolbar,'off'))
            ...
            if (colorbarthickness~=1)
               % David : 'CBThickness' option (does not work ???)
                theaxes=get(hh(1),'Parent');
                x1=get(theaxes,'position');
                x=get(hcb,'Position');
                if contains(lower(showcolbar),'north')||contains(lower(showcolbar),'south')
                    x(4) = colorbarthickness*x(4);
                else
                    x(3) = colorbarthickness*x(3);
                end
                set(hcb,'Position',x);
                set(theaxes,'position',x1);
                x1=get(theaxes,'position');
            end
            hh=[hcb; hh];
            % David : 'logsat' option for logarithmically stretched colorrange
            if(logscaleS ~= -1)
                S = logscaleS; 
                Tmax = round(S*(exp(hcb.Ticks(end)).^(sign(hcb.Ticks(end))/S)-1),2,'significant');
                Tint = round(S*(exp(hcb.Ticks(end)/2).^(sign(hcb.Ticks(end)/2)/S)-1),2,'significant');
                ticks = [-Tmax -Tint 0 Tint Tmax];
                sTicks = 5;
...
function printhelp()
    ...
    fprintf('''ColorBar''     Indicator in order to include a colorbar (may be a location such as ''North'') \n');
    fprintf('''CBTitle''      Colorbar Title (default=[])\n');
    fprintf('''CBThickness''  Colorbar Thickness (default=[])\n');
